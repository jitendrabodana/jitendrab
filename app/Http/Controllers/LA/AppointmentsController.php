...
class AppointmentsController extends Controller
{
	...
	public $view_col = 'city';
	public $listing_cols = ['id', 'date', 'time', 'car_model', 'state', 'city', 'name', 'email'];
		...
		if(Module::hasAccess("Appointments", "view")) {
			...
			if(isset($appointment->id)) {
				...
				return view('errors.404', [
					'record_id' => $id,
					'record_name' => ucfirst("appointment"),
				]);
			}
		} else {
			return redirect(config('laraadmin.adminRoute')."/");
	...
	{
		$values = DB::table('appointments')->select($this->listing_cols)->whereNull('deleted_at');
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('laraadmin.adminRoute') . '/appointments/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Appointments", "edit")) {
					$output .= '<a href="'.url(config('laraadmin.adminRoute') . '/appointments/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}
